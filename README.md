## Hi, Welcome to my GitHub Profile! 👋

I'm a Mathematics graduate who likes finding the most direct route
from a messy real-world problem to a working solution — usually involving
some combination of data, code, and stubbornness.

Most recently I built a full end-to-end mobile app for a New Zealand
membership scheme: I wrote a scraper to get the data,
built and hosted backend to serve it, shipped a React Native app to both the
App Store and Google Play, and debugged the parts that broke along the way.
That kind of full-pipeline ownership is what I enjoy most.

I'm based in New Zealand — open to opportunities.

Check out my [Portfolio](https://felixmraasch.github.io) for all of my projects.

**Connect:** [LinkedIn](https://linkedin.com/in/felixraasch)
